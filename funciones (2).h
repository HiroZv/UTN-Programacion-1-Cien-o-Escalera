...
bool esEscalera(int v[], int tam)
{
    if (tam <= 1) return true;  // Un vector de 0 o 1 elemento siempre es escalera

    ordenarVector(v, tam);  // Asumimos que esta función ordena el vector correctamente

    for (int i = 1; i < tam; i++)
    {
        if (v[i] != v[i-1] + 1)
        {
            return false;  // Si encontramos un número que no es consecutivo, no es escalera
        }
    }
    return true;  // Si llegamos aquí, todos los números son consecutivos
}
